...
for i=1:n_epsilon
    epsilon_range(i) = epsilon_range(i)*1.35^(i+4);
end
...
index_gamma = 3;
gamma_set = [2,3,6,12];
gamma = gamma_set(index_gamma); %modify the index for different CD frequency
...
filename = strcat('trial_gamma_',num2str(index_gamma),'.mat');
save(filename)
